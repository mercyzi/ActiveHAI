...
import warnings
from utils import load_data, save_json, load_json
import json
from layers import Agent, Agent_ECH
...
# load dataset
train_samples, test_samples, dev_samples = load_data(train_path), load_data(test_path), load_data(dev_path)
record = {}

train_size, test_size, dev_size = len(train_samples), len(test_samples), len(dev_samples)
...
notrain_ds_loader = DataLoader(train_ds, batch_size=train_size, num_workers=0, shuffle=False, collate_fn=lm_collater)

dev_ds = SymptomDataset(dev_samples, sv, dv, keep_unk=False, add_tgt_start=True, add_tgt_end=True)
dev_ds_loader = DataLoader(dev_ds, batch_size=dev_size, num_workers=num_workers, shuffle=False, collate_fn=lm_collater)
...
# Three doctors with different abilities: from low to high
for human_acc in [0.65, 0.7, 0.75]:
    human_pred_eva_all = load_json(f"../../data/MZ-10/all_eva_{human_acc}.json")
    doctor_name = str(human_acc)+'_doctor' 
    print("-" * 50+str(doctor_name)+"-" * 50)
    repeat_acc_hm_list = []
    repeat_acc_m_list = []
    repeat_acc_h_list = []
    # AI model pretrained 'num_repeats' times repeatedly
    for repeat_i in range(num_repeats):
        train_dataset = CustomDataset(f"../../data/MZ-10/{doctor_name}/model_output_{repeat_i}_train.csv")
        test_dataset = CustomDataset(f"../../data/MZ-10/{doctor_name}/model_output_{repeat_i}_test.csv")
        ...
        with torch.no_grad():
            for data in notrain_loader:
                ...
                human = HumanAgent(notrain_ds_loader, dev_ds_loader, test_ds_loader, human_acc=human_acc)
        ...
        for human_repeat_id in range(num_human_repeats):
            ...
            for nums_limit in limit_human_pred:
                ...
                if nums_limit == limit_human_pred[-1]:
                    ...
                    epochs = 100
                optimizer = torch.optim.Adam(evaluator_model.parameters(), lr = 3e-4)
                ...
                for epoch in range(epochs):
                    ...
                    test_record_temp_h.append(h_outputs)
                ...
                with torch.no_grad():
                    acc_list = torch.tensor([0.5, 0.5, 0.5, 0.5, 0.5,0.5, 0.5, 0.5, 0.5, 0.5])
                ...
                if nums_limit != limit_human_pred[-1]:
                    ...
                    perfect_h_all = torch.tensor(human_pred_eva_all[str((repeat_i, human_repeat_id))])
