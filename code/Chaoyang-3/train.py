...
for repeat_i in range(num_repeats):
    train_dataset = CustomDataset(f"../../data/Chaoyang-3/model_output_{repeat_i}_train.csv")
    test_dataset = CustomDataset(f"../../data/Chaoyang-3/model_output_{repeat_i}_test.csv")
    ...
    for human_repeat_id in range(num_human_repeats):
        ...
        for nums_limit in limit_human_pred:
            ...
            with torch.no_grad():
                acc_list = torch.tensor([0.5, 0.50, 0.5])
                if nums_limit < limit_human_pred[-2] and random_mode != True:
                    ...
                    for sample in al_sample_loader:
                        logits, labels, _, feat = sample
                        logits = logits.to(device)
                        labels = labels#.to(device)
                        h_labels = labels.to(device)
                        feat = feat.to(device)
                        calibrated_model_probs = calibrator.calibrate(logits)
